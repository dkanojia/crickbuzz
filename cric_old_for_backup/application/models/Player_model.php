...
class Player_model extends CI_Model {
	...
	function getplayer(){
		...
		$ii = 1;
		foreach($data as $key => $value){
			$str = $str . '<tr><td>'.$ii.'</td><td>'.$value['name'].'</td><td>'.$value['role'].'</td><td>'.$value['dob'].'</td><td>'.$value['mobile'].'</td><td>'.$value['city'].'</td><td>'.$value['bat'].'</td><td>'.$value['bowler'].'</td><td>'.$value['team'].'</td><td><button class ="btn btn-warning btn-xs"><a href = "edit_player/'.$value['pid'].'">Edit</a></button><button class ="btn btn-danger btn-xs"><a href = "delete_player/'.$value['pid'].'">Delete</a></button></td></tr>';
			$ii++;
		}
		 //$str;
		return $str;
